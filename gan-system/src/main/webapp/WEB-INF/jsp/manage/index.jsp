...
<head>
	...
	<style>
		/** skins **/
		<c:forEach var="SysSystem" items="${SysSystems}">
		#${SysSystem.name} #header {background: ${SysSystem.theme};}
		#${SysSystem.name} .content_tab{background: ${SysSystem.theme};}
		#${SysSystem.name} .s-profile>a{background: url(${basePath}${SysSystem.banner}) left top no-repeat;}
		</c:forEach>
	</style>
</head>
<body>
			...
			<ul class="hi-menu">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu dm-icon pull-right">
						...
						<c:forEach var="SysSystem" items="${SysSystems}">
						<li>
							<a class="waves-effect switch-systems" href="javascript:;" systemid="${SysSystem.systemId}" systemname="${SysSystem.name}" systemtitle="${SysSystem.title}"><i class="${SysSystem.icon}"></i> ${SysSystem.title}</a>
						</li>
						</c:forEach>
					</ul>
				</li>
	...
	<aside id="sidebar">
		...
		<div class="s-profile">
			<a class="waves-effect waves-light" href="javascript:;">
				<div class="sp-pic">
					<img src="${basePath}${SysUser.avatar}"/>
				</div>
				<div class="sp-info">
					${SysUser.realname}，您好！
		...
		<ul class="main-menu">
			...
			<c:forEach var="SysPermission" items="${SysPermissions}" varStatus="status">
				<c:if test="${SysPermission.pid == 0}">
				<li class="sub-menu system_menus system_${SysPermission.systemId} ${status.index}" <c:if test="${SysPermission.systemId != 1}">style="display:none;"</c:if>>
					<a class="waves-effect" href="javascript:;"><i class="${SysPermission.icon}"></i> ${SysPermission.name}</a>
					<ul>
						<c:forEach var="subSysPermission" items="${SysPermissions}">
							<c:if test="${subSysPermission.pid == SysPermission.permissionId}">
								<c:forEach var="SysSystem" items="${SysSystems}">
									<c:if test="${subSysPermission.systemId == SysSystem.systemId}">
									<c:set var="systemBasePath" value="${SysSystem.basepath}"/></c:if>
								</c:forEach>
			...
			<li>
				<div class="Sys-version">&copy; ZHENG-Sys V1.0.0</div>
			</li>
		</ul>
		<!-- /菜单区 -->
	</aside>
