	...
	<form id="organizationForm" method="post">
		<div class="form-group">
			<select id="organizationId" name="organizationId" multiple="multiple" style="width: 100%">
				<c:forEach var="SysOrganization" items="${SysOrganizations}">
					<option value="${SysOrganization.organizationId}" <c:forEach var="SysUserOrganization" items="${SysUserOrganizations}"><c:if test="${SysOrganization.organizationId==SysUserOrganization.organizationId}">selected="selected"</c:if></c:forEach>>${SysOrganization.name}</option>
				</c:forEach>
			</select>
		</div>
