		...
		<div class="form-group">
			<select id="roleId" name="roleId" multiple="multiple" style="width: 100%">
				<c:forEach var="SysRole" items="${SysRoles}">
					<option value="${SysRole.roleId}" <c:forEach var="SysUserRole" items="${SysUserRoles}"><c:if test="${SysRole.roleId==SysUserRole.roleId}">selected="selected"</c:if></c:forEach>>${SysRole.title}</option>
				</c:forEach>
			</select>
		</div>
